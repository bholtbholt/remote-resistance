...
<div id="TeamBuildingForm" in:fade>
  ...
  <form on:submit|preventDefault="{confirmTeam}">
    <div class="grid {gridSize($ruleset.playerCount)} -mx-lg mb-xl gap-xs border solid border-gray-800">
    {#each $players.map(player => { return { selected: $team.includes(player.id), ...player } }) as player}
      <label for="player_{player.id}"
        class="relative text-center bg-white rounded-sm ease-in duration-75 transition-shadow cursor-pointer"
